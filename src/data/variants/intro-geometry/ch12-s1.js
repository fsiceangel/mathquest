...
const s121 = [
  ...
  [
    {
      ...
      answer: 3,
      solution:
        'Route one, run the halving rule backward: an arc is double the inscribed angle standing on it, so $\\overset{\\frown}{AB} = 2 \\times 27^\\circ = 54^\\circ$ ✓. Check a second, independent way, by testing the candidate against the whole circle: an arc of $54^\\circ$ leaves a major arc of $360^\\circ - 54^\\circ = 306^\\circ$, and every point on that major arc — the footbridge included — must see the $54^\\circ$ arc at $\\frac{54^\\circ}{2} = 27^\\circ$, exactly the angle marked, so $54^\\circ$ is the only arc that fits the picture ✓. (The choice $13.5^\\circ$ is HALVING WHEN THE RULE SAYS DOUBLE ✗; the choice $27^\\circ$ is CONFUSING THE ARC WITH THE ANGLE ✗; the choice $108^\\circ$ is DOUBLING TWICE ✗.)',
    },
    {
    ...
    },
    {
      ...
      answer: 0,
      solution:
        'Route one, run the halving rule backward: the arc is twice the inscribed angle standing on it, so $\\overset{\\frown}{AB} = 2 \\times 46^\\circ = 92^\\circ$ ✓. Check a second, independent way, by testing the candidate against the whole circle: an arc of $92^\\circ$ leaves a major arc of $360^\\circ - 92^\\circ = 268^\\circ$, and every point on that major arc — the rider’s horse included — sees the $92^\\circ$ arc at $\\frac{92^\\circ}{2} = 46^\\circ$, exactly the marked angle, so $92^\\circ$ is the only arc that fits ✓. (The choice $46^\\circ$ is CONFUSING THE ARC WITH THE ANGLE ✗; the choice $184^\\circ$ is DOUBLING TWICE ✗; the choice $23^\\circ$ is HALVING WHEN THE RULE SAYS DOUBLE ✗.)',
    },
  ],
  ...
  [
    ...
    },
    {
      ...
      answer: 3,
      solution:
        'Route one, the semicircle rule: $A$, $O$, $B$ collinear with $O$ the center means $AB$ is a diameter, so the arc from $A$ to $B$ on the far side from $C$ is a semicircle of $180^\\circ$, and the inscribed angle at $C$ is half of it: $\\angle ACB = \\frac{180^\\circ}{2} = 90^\\circ$ ✓. Check a second, independent way, with radii only: $OA = OC = OB$, so triangle $AOC$ is isosceles with base angles $x$ at $A$ and $C$, and triangle $BOC$ is isosceles with base angles $y$ at $B$ and $C$. Triangle $ABC$ then has angles $x$, $y$, and $x + y$, and $2x + 2y = 180^\\circ$ forces $\\angle ACB = x + y = 90^\\circ$ ✓. (The choice $60^\\circ$ is ASSUMING THE TRIANGLE IS EQUILATERAL ✗; the choice $180^\\circ$ is CONFUSING THE ARC WITH THE ANGLE ✗; the choice $120^\\circ$ is SPLITTING THE CIRCLE INTO THREE EQUAL PIECES, as if $C$ were a third of the way around ✗.)',
    },
    {
  ...
  [
    ...
    },
    {
      ...
      answer: 3,
      solution:
        'Route one, pick the right arc: the angle at tent $A$ opens across the field onto arc $BC$, the arc that does not contain $A$, so $\\angle A = \\frac{96^\\circ}{2} = 48^\\circ$ ✓. Check a second, independent way, by building the whole triangle and testing the angle sum: $\\angle B = \\frac{174^\\circ}{2} = 87^\\circ$ from arc $CA$ and $\\angle C = \\frac{90^\\circ}{2} = 45^\\circ$ from arc $AB$, and $48^\\circ + 87^\\circ + 45^\\circ = 180^\\circ$, so the three angles fit a real triangle and $48^\\circ$ stands ✓. (The choice $45^\\circ$ is USING ARC $AB$, an arc that touches $A$ instead of facing it ✗; the choice $96^\\circ$ is CONFUSING THE ARC WITH THE ANGLE ✗; the choice $87^\\circ$ is USING ARC $CA$, the other arc touching $A$ ✗.)',
    },
  ],
