  ...
  [
    {
      ...
      answer: 3,
      solution:
        'A dilation stretches both dimensions, so areas are multiplied by $k^2 = (-3)^2 = 9$, and the image covers $6 \\cdot 9 = 54$ square units ✓. Check a second, independent way by building a triangle with the stated area: a base of $4$ and a height of $3$ give area $\\frac{1}{2}(4)(3) = 6$. The dilation multiplies every length by $|-3| = 3$, so the base becomes $12$ and the height becomes $9$, and the image has area $\\frac{1}{2}(12)(9) = 54$ ✓. (The choice $18$ is SCALING THE AREA BY $|k|$ INSTEAD OF $k^2$, which is how lengths scale, not areas ✗; the choice $9$ is REPORTING THE AREA MULTIPLIER $k^2$ RATHER THAN THE AREA ITSELF ✗; the choice $-54$ is LETTING THE NEGATIVE SIGN THROUGH, but the minus only flips the sail to the far side of the center and no region has negative area ✗.)',
    },
    {
  ...
  [
    {
      ...
      answer: 1,
      solution:
        'Read the slide off the matched pair: from $5$ to $1$ is $-4$ across, and from $-2$ to $3$ is $+5$ up, so the rule is $(x, y) \\to (x - 4, y + 5)$. Applying it to the lighthouse gives $(6 - 4, -1 + 5) = (2, 4)$ ✓. Check a second, independent way without ever writing the rule: a translation carries the whole map rigidly, so the lighthouse keeps its position relative to the town. The lighthouse sits $1$ unit right and $1$ unit up from the town, so its image sits $1$ unit right and $1$ unit up from $(1, 3)$, at $(2, 4)$ ✓. (The choice $(11, -5)$ is SWAPPING THE TWO COMPONENTS OF THE SLIDE, moving $5$ across and $4$ up or down ✗; the choice $(10, -6)$ is RUNNING THE SLIDE BACKWARDS, $4$ right and $5$ down ✗; the choice $(-4, 5)$ is REPORTING THE SLIDE ITSELF instead of the image of the lighthouse ✗.)',
    },
    {
