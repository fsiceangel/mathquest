...
const s33 = [
  ...
  [
    ...
    },
    {
      ...
      answer: 3,
      solution:
        'The marked sides of the left triangle are $\\overline{EF}$ and $\\overline{EG}$, which share the endpoint $E$, and the $64^\\circ$ angle is drawn at $E$ — right between them. In the right triangle the marked sides $\\overline{RS}$ and $\\overline{RT}$ share $R$, and the $64^\\circ$ angle is drawn at $R$. Two sides and the included angle: SAS ✓. Check a second, independent way by testing the marked angle against the definition of "included" rather than trusting the picture. The angle at $E$ has arms $\\overline{EF}$ and $\\overline{EG}$; both of those carry tick marks, and the unmarked side $\\overline{FG}$ is not an arm of it. An angle whose two arms are exactly the two marked sides is by definition the included angle ✓, so the SSA arrangement — where one arm of the angle is the unmarked side — is ruled out ✗. Three side pairs are not marked, so SSS is out ✗, and no second angle is marked, so ASA is out ✗. (Choosing SSS or ASA is COUNTING MARKS THAT ARE NOT THERE; choosing SSA is CHOOSING A NON-INCLUDED ANGLE.)',
    },
  ],
  ...
  [
    ...
    },
    {
      ...
      answer: 2,
      solution:
        'Set up the congruence first. Bisecting gives $GN = KN$ and $HN = LN$; the angles $\\angle GNH$ and $\\angle KNL$ are vertical angles, so they are equal, and each lies between the two half-segments of its own triangle. By SAS, $\\triangle NGH \\cong \\triangle NKL$ with $G$ matched to $K$ and $H$ matched to $L$, so $\\angle NKL = \\angle NGH = 62^\\circ$ ✓. Check a second, independent way with the half turn about $N$: it carries $G$ to $K$ and $H$ to $L$, so it carries $\\angle NGH$ onto $\\angle NKL$, and a rigid turn leaves every angle the size it was ✓. Notice what the answer is not — the two angles are equal rather than supplementary, because the half turn matches them up directly instead of leaving them side by side along a line. (The choice $124^\\circ$ is DOUBLING THE ANGLE ✗; $28^\\circ$ is TREATING THE ANGLES AS COMPLEMENTARY ✗; $118^\\circ$ is TREATING THE ANGLES AS SUPPLEMENTARY ✗.)',
    },
  ],
]
