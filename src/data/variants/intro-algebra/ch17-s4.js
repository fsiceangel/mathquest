...
const challenge = [
  ...
  [
    ...
    },
    {
      ...
      answer: 2,
      solution:
        'The graph of $y = |x + 5| - 8$ is a V with its corner at $(-5, -8)$, opening upward. The line $y = -3$ sits $5$ units ABOVE that corner, so it cuts through both arms: $2$ solutions. Algebra agrees: $|x + 5| = 5$ means the distance from $x$ to $-5$ is $5$, giving $x = 0$ and $x = -10$; check both, $|0 + 5| - 8 = -3$ ✓ and $|-10 + 5| - 8 = -3$ ✓, and they are genuinely different numbers. (The choice $0$ decides that a negative right-hand side is impossible, missing that the $-8$ has already dragged the corner down to $-8$, far below $-3$. The choice $1$ solves only the branch $x + 5 = 5$ and forgets the arm on the left. The choice "infinitely many" reads the equation as though every $x$ between $-10$ and $0$ worked, which is what an inequality would give.)',
    },
    {
  ...
  [
    {
      ...
      answer: 1,
      solution:
        'Walk the graph from left to right and count every meeting with the dashed line. The graph falls from $(-4, 5)$ to $(-1, -3)$, starting above the line and finishing below it, so it must cross once. It then rises only as far as $(1, 1)$ — one unit SHORT of the line — and falls again to $(3, -2)$, so that whole hump contributes nothing. Finally it climbs from $(3, -2)$ to $(4, 4)$, passing the line a second time. That is $2$ solutions. Check the endpoints, which are the easiest meetings to miss: the graph starts at height $5$ and ends at height $4$, and neither equals $2$ ✓, so nothing extra hides at the ends. (The choice $1$ stops after the first crossing and misses the final climb. The choice $3$ counts the peak at $(1, 1)$ as a touch, but $1 < 2$ and the graph never reaches the line there. The choice $4$ assumes each of the four straight pieces meets the line once.)',
    },
    {
      ...
      answer: 2,
      solution:
        'Count the meetings piece by piece, using the corner points. Falling from $(-5, 3)$ to $(-2, -3)$ the graph passes the line once, at $x = -3$. Rising from $(-2, -3)$ to $(1, 3)$ it passes again, at $x = -1$. Falling from $(1, 3)$ to $(4, -3)$ it passes a third time, at $x = 3$. Then the short steep piece from $(4, -3)$ up to $(5, 0)$ starts below the line and ends above it, so it crosses a fourth time. That is $4$ inputs. Check the three named crossings by arithmetic: the first piece drops $2$ per step, so from $3$ it reaches $-1$ after $2$ steps, at $x = -3$ ✓; the second rises $2$ per step, reaching $-1$ one step after $-2$, at $x = -1$ ✓; the third falls $2$ per step from $(1, 3)$, reaching $-1$ two steps later, at $x = 3$ ✓. (The choice $2$ counts only the two downward crossings and skips the upward ones. The choice $3$ misses the crossing on the short final piece, which is easy to overlook because it is steep. The choice $5$ counts the right-hand endpoint $(5, 0)$ as a meeting, but its height is $0$, not $-1$.)',
    },
    {
