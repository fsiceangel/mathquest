...
const s43 = [
  ...
  [
    ...
    },
    {
      ...
      answer: 2,
      solution:
        'Route one, clear the denominator. $\\frac{n(n-1)(n-2)}{6} = 286$ gives $n(n-1)(n-2) = 1716$. Three consecutive integers with that product sit near $\\sqrt[3]{1716} \\approx 12$, and $13 \\cdot 12 \\cdot 11 = 1716$ exactly, so $n = 13$ ✓. Route two, build a table with the splitting rule. A group of three either uses a fixed object or not, so $\\binom{n}{3} = \\binom{n-1}{2} + \\binom{n-1}{3}$. Starting from $\\binom{10}{3} = 120$ and $\\binom{10}{2} = 45$: $\\binom{11}{3} = 165$; then $\\binom{11}{2} = 55$ gives $\\binom{12}{3} = 220$; then $\\binom{12}{2} = 66$ gives $\\binom{13}{3} = 286$ ✓. Each step adds objects, so the counts only rise and $n = 13$ is the only answer ✓. (The choice $12$ STOPS ONE SHORT, since $\\binom{12}{3} = 220$ ✗; $1716$ REPORTS THE PRODUCT $n(n-1)(n-2)$ INSTEAD OF $n$ ✗; $14$ GOES ONE TOO FAR, since $\\binom{14}{3} = 364$ ✗.)',
    },
  ],
  ...
  [
    ...
    },
    {
      ...
      answer: 0,
      solution:
        'Route one, divide out the part you know. The birds contribute $\\binom{8}{3} = \\frac{8 \\cdot 7 \\cdot 6}{6} = 56$ choices, so $56 \\cdot \\binom{n}{2} = 840$ and $\\binom{n}{2} = 15$. Then $\\frac{n(n-1)}{2} = 15$ gives $n(n-1) = 30 = 6 \\cdot 5$, so $n = 6$ ✓. Route two, try values and watch the total climb. With $n$ rabbits the group count is $56 \\binom{n}{2}$, which rises as $n$ rises because a new rabbit only adds pairs: $n = 4$ gives $56 \\cdot 6 = 336$; $n = 5$ gives $56 \\cdot 10 = 560$; $n = 6$ gives $56 \\cdot 15 = 840$ ✓; $n = 7$ gives $56 \\cdot 21 = 1176$, already past the target. So $n = 6$ is the only answer ✓. (The choice $7$ OVERSHOOTS, giving $1176$ groups ✗; $15$ REPORTS $\\binom{n}{2}$ INSTEAD OF $n$ ✗; $5$ FALLS SHORT, giving $560$ groups ✗.)',
    },
  ],
