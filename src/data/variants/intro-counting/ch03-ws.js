...
const worksheet = [
  ...
  [
    ...
    },
    {
      ...
      answer: '$28$',
      solution:
        'Set up and solve. With $n$ teams, counting games from the teams’ side gives $6n$, and each game gets counted twice because two teams play in it. So $\\frac{6n}{2} = 84$, that is $3n = 84$ and $n = 28$. Since $3n = 84$ has only one solution, the answer is settled, and $28$ is large enough for each team to have $6$ different opponents ✓. Second route, through the pairs that have not met, one league size at a time. In a league of $n$ teams there are $\\frac{n(n-1)}{2}$ pairs altogether, and each team is still waiting on $n - 7$ opponents, so $\\frac{n(n-7)}{2}$ pairs have not met; the games played are the pairs left over. A league of $20$ teams gives $190 - 130 = 60$ games, one of $24$ gives $276 - 204 = 72$, one of $28$ gives $378 - 294 = 84$ — a hit — and one of $32$ would give $496 - 400 = 96$. Adding a team never lowers the count, so $84$ is reached at exactly one league size, and it is $28$ ✓. DIVIDING $84$ BY $6$ TO GET $14$ forgets that each game is shared by two teams; $14$ teams playing $6$ games apiece would account for only $\\frac{14 \\cdot 6}{2} = 42$ games ✗.',
    },
  ],
