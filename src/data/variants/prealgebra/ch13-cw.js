...
const challenge = [
  ...
  [
    {
      ...
      choices: ['$4$', '$32$', '$28$', '$112$'],
      answer: 1,
      solution: 'Mean $\\times$ count $=$ total, so four numbers with mean $28$ must total $4 \\times 28 = 112$. The three known numbers give $18 + 27 + 35 = 80$, so $y = 112 - 80 = 32$. Check: $\\frac{18 + 27 + 35 + 32}{4} = \\frac{112}{4} = 28$. ✓ ($4$ comes from multiplying by the $3$ known numbers instead of all $4$, giving $84 - 80$; $28$ assumes the missing number must equal the mean; $112$ is the total, which still needs the subtraction.)',
    },
    {
      q: 'The mean of $42$, $19$, $23$, $30$, and $m$ is $30$. What is $m$?',
      choices: ['$6$', '$114$', '$36$', '$30$'],
      answer: 2,
      solution: 'Five numbers with mean $30$ must total $5 \\times 30 = 150$. The four known numbers give $42 + 19 + 23 + 30 = 114$, so $m = 150 - 114 = 36$. Check: $\\frac{114 + 36}{5} = \\frac{150}{5} = 30$. ✓ ($6$ comes from using the $4$ known values as the count, $120 - 114$; $114$ is the running total of what we already knew; $30$ assumes the missing number equals the mean.)',
    },
    {
      q: 'Nadia scored $93$, $84$, and $87$ on three science labs. What must she score on a fourth lab so that her mean is exactly $90$?',
      choices: ['$84$', '$88$', '$90$', '$96$'],
      answer: 3,
      solution: 'Four labs with mean $90$ must total $4 \\times 90 = 360$. So far she has $93 + 84 + 87 = 264$, so she needs $360 - 264 = 96$. The surplus method agrees: her scores sit $+3$, $-6$, and $-3$ from $90$, a net shortfall of $6$, so the fourth lab must be $6$ above $90$. ($84$ flips the sign of the shortfall and subtracts the $6$ instead of adding it; $88$ is the mean of just the three labs she has already taken; $90$ assumes the last score equals the target mean.)',
    },
  ],
  ...
  [
    {
      ...
      choices: ['$79$', '$75$', '$85$', '$91$'],
      answer: 0,
      solution: 'Count the points, not the scores: $8 \\times 75 + 2 \\times 95 = 600 + 190 = 790$, over $10$ quizzes, so the mean is $\\frac{790}{10} = 79$. ($75$ ignores the two high quizzes entirely; $85$ is the average of the two averages, $\\frac{75 + 95}{2}$ — that would only be right with equal numbers of each score, and there are four times as many $75$s; $91$ attaches the counts to the wrong scores, $\\frac{2 \\times 75 + 8 \\times 95}{10}$.)',
    },
    {
      ...
      choices: ['$8$', '$11$', '$14$', '$17$'],
      answer: 1,
      solution: 'Multiply each value by its frequency, then add: $6 \\times 8 + 3 \\times 14 + 1 \\times 20 = 48 + 42 + 20 = 110$ books. Divide by the number of MEMBERS, $6 + 3 + 1 = 10$, not by the three different book counts: $\\frac{110}{10} = 11$. ($8$ is the mode, the most common value, not the mean; $14$ is $\\frac{8 + 14 + 20}{3}$, the average of the three values with the frequencies thrown away; $17$ pairs the frequencies with the wrong values, $\\frac{1 \\times 8 + 3 \\times 14 + 6 \\times 20}{10}$.)',
    },
    {
      ...
      choices: ['$81$', '$85$', '$79$', '$80$'],
      answer: 2,
      solution: 'Rebuild the totals: Section A holds $12 \\times 85 = 1020$ points and Section B holds $18 \\times 75 = 1350$ points. Together that is $2370$ points among $30$ students: $\\frac{2370}{30} = 79$. ($81$ swaps the two group sizes; $85$ forgets Section B is even there; $80$ averages the averages, $\\frac{85 + 75}{2}$ — but Section B is the bigger group, so the combined mean must be pulled below the midpoint, toward $75$.)',
    },
  ],
  ...
  [
    ...
    },
    {
      ...
      choices: ['$24$', '$29$', '$34$', '$49$'],
      answer: 1,
      solution: 'The total must be $6 \\times 9 = 54$. Drop the other FIVE numbers to their minimum of $5$, using $5 \\times 5 = 25$, leaving $54 - 25 = 29$. Check: $5, 5, 5, 5, 5, 29$ totals $54$, giving a mean of $9$. ✓ ($24$ subtracts six $5$s, one too many — the big number is not also sitting at the floor; $34$ shrinks only four of the others; $49$ sets aside a single $5$.)',
    },
    {
      ...
      choices: ['$0$', '$18$', '$30$', '$10$'],
      answer: 3,
      solution: 'This time the limit points the other way. The total is $5 \\times 18 = 90$, and to leave one number as small as possible the other FOUR should be as big as they are allowed: $4 \\times 20 = 80$. That leaves $90 - 80 = 10$. Check: $20, 20, 20, 20, 10$ totals $90$. ✓ And $9$ is impossible, since the other four could then reach only $80 < 81$. ($0$ forgets that the others are capped at $20$; $18$ is the all-equal set, which satisfies the rules but is not the smallest; $30$ pushes only three of them to the cap.)',
    },
  ],
  ...
  [
    {
      ...
      choices: ['$2$', '$0$', '$6$', '$10$'],
      answer: 0,
      solution: 'Books read: $3 \\times 0 + 5 \\times 2 + 5x = 10 + 5x$. Students: $3 + 5 + x = 8 + x$. A mean of $2$ means the books are twice the students: $10 + 5x = 2(8 + x) = 16 + 2x$, so $3x = 6$ and $x = 2$. Check: $10$ students read $0 + 10 + 10 = 20$ books, a mean of exactly $2$. ✓ ($0$ assumes nobody can sit above the mean, which is not so — the students who read $0$ balance them; $6$ stops at the line $3x = 6$ without dividing; $10$ reports the total number of students instead.)',
    },
    {
      ...
      choices: ['$0$', '$10$', '$15$', '$5$'],
      answer: 3,
      solution: 'Points: $5 \\times 4 + 5 \\times 7 + 10y = 55 + 10y$. Students: $10 + y$. Setting the mean to $7$ gives $55 + 10y = 7(10 + y) = 70 + 7y$, so $3y = 15$ and $y = 5$. Check: $15$ students scored $20 + 35 + 50 = 105$ points, and $\\frac{105}{15} = 7$. ✓ Balance view: five scores sit $3$ below the mean, a deficit of $15$, and each $10$ supplies a surplus of $3$, so five of them are needed. ($0$ assumes no score may exceed the mean; $10$ reports the students already listed; $15$ stops at $3y = 15$.)',
    },
    {
  ...
  [
    ...
    },
    {
      ...
      choices: ['$14$', '$21$', '$20$', '$28$'],
      answer: 2,
      solution: 'Sorted, the set is $5, 5, 7, a, b$, with total $5 \\times 9 = 45$, so $a + b = 45 - 17 = 28$. Since a second $7$ would tie the mode, $a \\ge 8$, which caps $b$ at $28 - 8 = 20$. The set $5, 5, 7, 8, 20$ has mode $5$, median $7$, and mean $\\frac{45}{5} = 9$. ✓ ($14$ splits the $28$ evenly, but $5, 5, 7, 14, 14$ is bimodal too; $21$ comes from allowing $a = 7$, which gives the set two modes; $28$ is the total of the top two, not a single value.)',
    },
  ],
]
