...
const s152 = {
  ...
  problems: [
    ...
    {
      q: 'Four standard dice are rolled. What is the probability that the four numbers rolled are all different, the largest of them is $6$, and their sum is even?',
      choices: ['$\\frac{1}{9}$', '$\\frac{5}{54}$', '$\\frac{5}{27}$', '$\\frac{2}{27}$'],
      answer: 0,
      solution: 'One outcome is an ordered list of four rolls, $6^4 = 1296$ in all, and each qualifying outcome is one of the $4! = 24$ orders of a qualifying set of values. A qualifying set contains $6$ and three different values from $1$ through $5$, and since $6$ is even, the sum is even exactly when those three values have an even sum — that is, exactly two of them are odd. Choose $2$ of the odds $1, 3, 5$ and $1$ of the evens $2, 4$: $\\binom{3}{2} \\cdot \\binom{2}{1} = 6$ sets, so $6 \\cdot 24 = 144$ outcomes and the probability is $\\frac{144}{1296} = \\frac{1}{9}$. Check by counting the odd sums in the same family: one odd with both evens gives $3$ sets and all three odds give $1$ more, so $4 \\cdot 24 = 96$ outcomes, and $144 + 96 = 240$ matches the count of all-different-with-largest-$6$ rolls. $\\frac{5}{54}$ assumes the even and odd sums split evenly, $\\frac{5}{27}$ drops the parity condition entirely, and $\\frac{2}{27}$ counts the odd sums instead.',
    },
    {
    ...
    {
      q: 'A fair coin is flipped $7$ times. What is the probability that the first flip is heads and the total number of heads is a multiple of $3$?',
      choices: ['$\\frac{15}{128}$', '$\\frac{43}{256}$', '$\\frac{21}{128}$', '$\\frac{21}{64}$'],
      answer: 2,
      solution: 'One outcome is a sequence of $7$ flips, $2^7 = 128$ in all. With the first flip forced to be heads, the total cannot be $0$, so the total is $3$ or $6$ heads, and the remaining $6$ flips must supply $2$ or $5$ heads: $\\binom{6}{2} + \\binom{6}{5} = 15 + 6 = 21$ sequences, probability $\\frac{21}{128}$. Cross-check from the other side: of the $43$ sequences whose head count is a multiple of $3$ ($1 + 35 + 7$ for totals $0, 3, 6$), the ones starting with heads are $\\frac{3}{7} \\cdot 35 + \\frac{6}{7} \\cdot 7 = 15 + 6 = 21$, since a sequence with $h$ heads starts with heads in $\\frac{h}{7}$ of its arrangements. $\\frac{43}{256}$ multiplies the two conditions as if they were independent, $\\frac{21}{64}$ is the conditional probability given that the first flip is heads, and $\\frac{15}{128}$ keeps only the total of $3$.',
    },
    {
...
const challenge = [
  ...
  {
    q: 'A point is chosen uniformly at random inside a $3 \\times 2$ rectangle. What is the expected value of its distance to the nearest side of the rectangle?',
    choices: ['$\\frac{1}{3}$', '$\\frac{5}{12}$', '$\\frac{1}{2}$', '$\\frac{7}{18}$'],
    answer: 3,
    solution: 'Cut the rectangle along the fold lines where two sides tie for nearest: with corners at $(0, 0)$ and $(3, 2)$, the $45^\\circ$ folds from the corners meet the ridge segment from $(1, 1)$ to $(2, 1)$. That makes four regions. Bottom trapezoid, corners $(0, 0)$, $(3, 0)$, $(2, 1)$, $(1, 1)$: the distance is the height $y$, the area is $2$, and the average height of a trapezoid with parallel sides $3$ and $1$ is $\\frac{1}{3} \\cdot \\frac{3 + 2 \\cdot 1}{3 + 1} = \\frac{5}{12}$; the top trapezoid matches it. Left triangle, corners $(0, 0)$, $(1, 1)$, $(0, 2)$: the distance is $x$, the area is $1$, and the average of a coordinate over a triangle is that coordinate of the centroid, $\\frac{0 + 1 + 0}{3} = \\frac{1}{3}$; the right triangle matches. Weight the averages by area: the two trapezoids give $2 \\cdot 2 \\cdot \\frac{5}{12} = \\frac{5}{3}$ and the two triangles give $2 \\cdot 1 \\cdot \\frac{1}{3} = \\frac{2}{3}$, so the expected distance is $\\frac{1}{6}\\left(\\frac{5}{3} + \\frac{2}{3}\\right) = \\frac{7}{18}$. $\\frac{5}{12}$ pretends every point’s nearest side is the top or bottom, $\\frac{1}{3}$ carries over the answer for a square, where the four regions are congruent triangles, and $\\frac{1}{2}$ splits the difference between the ridge height $1$ and the sides at $0$.',
  },
  {
  ...
  {
    q: 'Eight cards numbered $1$ through $8$ are shuffled and laid out in a row from left to right. A card is a record if its number is larger than the number on every card to its left. What is the expected number of records among the last three cards of the row?',
    choices: ['$\\frac{1}{2}$', '$\\frac{73}{168}$', '$\\frac{761}{280}$', '$\\frac{11}{6}$'],
    answer: 1,
    solution: 'One piece per position. The card in position $k$ is a record exactly when it is the largest of the first $k$ cards, and by symmetry each of those $k$ cards is the largest equally often, so the probability is $\\frac{1}{k}$ — for the sixth, seventh, and eighth cards that is $\\frac{1}{6}$, $\\frac{1}{7}$, and $\\frac{1}{8}$. The pieces depend on one another, but expected values add: $\\frac{1}{6} + \\frac{1}{7} + \\frac{1}{8} = \\frac{28 + 24 + 21}{168} = \\frac{73}{168}$. As a check, a record in the last three positions is the card $8$ landing there, plus rarer double and triple records, and $\\frac{73}{168}$ sits just above $\\frac{3}{8}$, the chance that the $8$ alone lands late. $\\frac{11}{6}$ treats the last three cards as a fresh three-card deck and forgets the five cards already to their left, $\\frac{761}{280}$ is the expected number of records in the whole row, and $\\frac{1}{2}$ gives all three positions the sixth card’s $\\frac{1}{6}$.',
  },
]
